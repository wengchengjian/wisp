...
impl Node {
    ...
    /// 需要包裹 `<table>` 后用 `Document::parse` 解析，因为 HTML5 规范下这些表格元素
    ...
    pub fn from_fragment(html: &str) -> Self {
        ...
        if is_table_fragment {
            ...
            // parse_fragment 的 context 元素（div）挂在 Document 根下，片段内容直接在其下。
