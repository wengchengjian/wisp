...
use super::Node;
use wisp_storage::{ElementSnapshotRow, Store};
...
impl From<ElementSnapshot> for ElementSnapshotRow {
    fn from(s: ElementSnapshot) -> Self {
        Self {
            tag: s.tag,
...
impl From<ElementSnapshotRow> for ElementSnapshot {
...
pub fn css_adaptive(
    ...
    if let Some(node) = doc.select_one(selector) {
        ...
        if auto_save {
            ...
            let _ = wisp_storage::save_element(&*store, url, key, &snap.to_row(now));
        }
        return Some(node);
    }

    // 2. CSS failed - try relocate from saved snapshot
    let saved_row = wisp_storage::load_element(&*store, url, key)
        .ok()
        .flatten()?;
...
//
// 旧实现对每个候选节点调用 4 个 helper，每个 helper 都 outer_html() + 重新解析 HTML，
// 导致 similarity() 每次调用解析 4 次。现在改用 Node 的导航 API
