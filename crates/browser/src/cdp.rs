...
impl CdpSession {
    ...
    pub async fn connect(ws_url: &str) -> Result<Arc<Self>> {
        // 握手本身栈深不大，但会叠加调用方整条 async 调用链（如 Engine ->
        // fetch_browser -> Browser::launch），在 Windows 1MB 主线程栈上会溢出。
        // 拆到独立 task 后调用方只等待 JoinHandle，握手在 runtime worker 上执行。
