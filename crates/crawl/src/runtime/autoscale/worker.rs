...
impl AutoscaledPool {
    ...
    pub(crate) async fn run_autoscaler(self: &Arc<Self>, stats: Vec<Arc<SpiderStats>>) {
        ...
        loop {
            ...
            self.maybe_scale_down(error_rate, saturation).await;
            self.maybe_scale_up(error_rate, saturation).await;
        }
    }
}
