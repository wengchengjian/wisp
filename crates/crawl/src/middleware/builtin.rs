...
impl Middleware for CacheMiddleware {
    ...
    async fn process_request(&self, req: &mut Request, _ctx: &CrawlContext) -> MwAction {
        ...
        match wisp_storage::load_response(&*self.store, method_str, &req.url) {
            Ok(Some(cached)) => {
    ...
    async fn process_response(&self, resp: &mut Response, _ctx: &CrawlContext) -> MwAction {
        if resp.status >= 200 && resp.status < 400 && !resp.from_cache {
            ...
            if let Err(e) = wisp_storage::save_response(&*self.store, method_str, &resp.url, &cached) {
...
<script src='/c.js'></script>\
    ...
    fn default_middlewares_classifies_by_mode_and_config() {
        ...
        assert!(!auto_default_p.contains(&40), "默认 Auto 不应含 DynamicUpgrade");
    ...
    fn default_middlewares_names_match_config() {
        ...
        assert!(
            names.iter().any(|n| n.ends_with("DepthLimitMiddleware")),
            "缺少 DepthLimit: {names:?}"
        );
        assert!(
            names.iter().any(|n| n.ends_with("StealthUpgradeMiddleware")),
            "Auto 应含 StealthUpgrade: {names:?}"
        );
        assert!(
            names.iter().any(|n| n.ends_with("CookieChallengeMiddleware")),
        ...
        assert!(
            !names.iter().any(|n| n.ends_with("DynamicUpgradeMiddleware")),
