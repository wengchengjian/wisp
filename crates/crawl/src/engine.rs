...
async fn check_control_and_hook(
    ctx: &EngineContext,
    req: &Request,
...
pub(crate) async fn process_request(ctx: &EngineContext, req: Request) -> Option<Response> {
    ...
    if !spider.allowed_domains().is_empty() {
        let host = url::Url::parse(&req.url)
            ...
            .unwrap_or_default();
        let allowed = spider.allowed_domains().iter().any(|d| {
...
pub(crate) async fn process_response(ctx: &EngineContext, resp: Response) {
    ...
    let (items, follows) = spider
        .handle(resp)
        .instrument(tracing::trace_span!("spider.handle", spider = %spider.name()))
        .await;
...
async fn fetch_dispatch(ctx: &EngineContext, req: &Request) -> (Option<Response>, Option<String>) {
    let Some(stats) = ctx.state.stats_for(req) else {
        return (None, Some("request has no matching spider".to_string()));
    };
    let Some(spider) = ctx.state.spider_for(req) else {
        return (None, Some("request has no matching spider".to_string()));
    };
    let max_retries = ctx.config.max_retries;
    // 同步重试循环：成功路径不 clone，仅在升级/重试需要修改请求时才 clone。
    let current_req = req;
    let mut owned: Option<Request> = None;

    loop {
        let req_ref = owned.as_ref().unwrap_or(current_req);
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if ctx.config.fetch_mode == FetchMode::Auto
                    && req_ref.fetch_mode_override.is_none()
                    && req_ref.retry_count == 0
                    && ctx
                        .shared
                        .rule_engine
                        .lock()
                ...
                {
                    let url_for_log = sanitize_url(&req_ref.url);
                    ctx.shared
                        .rule_engine
                        .lock()
                        .await
                        .learn(&req_ref.url, FetchMode::Stealth);
                    tracing::info!(
                        "AutoFallback: '{}' 首次抓取失败 ({}), 升级 Stealth 重试",
                        url_for_log,
                        e
                    );
                    let mut upgraded = req_ref.clone();
                ...
                let action = if !ctx.shared.middleware_chain.is_empty() {
                    ...
                    ctx.shared
                        .middleware_chain
                        .run_error_middlewares(req_ref, &e.to_string(), &crawl_ctx)
                        .await
                } else {
                ...
                match action {
                    middleware::ErrorAction::Retry if req_ref.retry_count < max_retries => {
                        let retry_count = req_ref.retry_count + 1;
                        let url_for_log = sanitize_url(&req_ref.url);
                        let mut retried = req_ref.clone();
...
pub(crate) fn build_crawl_context(ctx: &EngineContext) -> middleware::CrawlContext {
    let spider = &ctx.state.spiders[0];
    let stats = &ctx.state.all_stats[0];
    build_crawl_context_for(ctx, spider, stats)
}

/// 从 EngineContext + 当前 Spider 构建中间件用的 CrawlContext 只读视图。
...
pub(crate) async fn persist_spider_checkpoint(
    ...
    wisp_storage::save_checkpoint(store, spider_name, &blob).map_err(|e| {
        wisp_core::error::WispError::Storage(wisp_core::error::StorageError::General(format!(
            "checkpoint 保存失败: {e}"
        )))
    })?;
...
pub async fn fetch_page(
    ...
    if let Some(override_mode) = req.fetch_mode_override {
        ...
        if override_mode == FetchMode::Stealth {
            ...
            if let Some(resp) =
                try_http_with_cf_cookie(fetch_client, req, proxy_url, proxy_clients).await?
            {
                tracing::info!(
    ...
    if mode == FetchMode::Auto {
        // 有 CF cookie：先尝试 HTTP（快速路径），被拦截则回退 Stealth
        if let Some(resp) =
            try_http_with_cf_cookie(fetch_client, req, proxy_url, proxy_clients).await?
        {
            tracing::info!(
        ...
        // 无可用 cookie 或 HTTP 被拦截，继续走 rule_engine / HTTP 先行路径。
...
pub async fn fetch_page_inner(
    ...
    if mode == FetchMode::Dynamic || mode == FetchMode::Stealth {
        ...
        let resp = match mode {
            FetchMode::Dynamic => {
                let strategy =
                    wisp_fetcher::DynamicStrategy::from_config(fetch_client.config());
