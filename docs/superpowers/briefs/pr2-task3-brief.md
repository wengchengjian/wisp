# PR2 Task 3: StealthStrategy 实现

**Files:**
- Create: `src/fetcher/strategies/stealth.rs`
- Modify: `src/fetcher/strategies/mod.rs`（启用 stealth 行）
- Modify: `src/fetcher/mod.rs`（启用 re-export）
- Test: `src/fetcher/strategies/stealth.rs`（内联测试模块）

**Interfaces:**
- Consumes: `crate::fetcher::strategy::{BrowserFetchStrategy, recv_navigation_status, extract_browser_response}`、`crate::browser::Page`、`crate::cookie::CfCookieJar`、`crate::cookie::CfSession`、`crate::stealth::{ChallengeSolver, HumanBehavior, TurnstileConfig}`、`crate::fetcher::client::FetchClientConfig`
- Produces: `pub struct StealthStrategy`；`impl StealthStrategy { pub fn from_config(config: &FetchClientConfig, cf_jar: Arc<CfCookieJar>) -> Self }`；`impl BrowserFetchStrategy for StealthStrategy`

## 关键修复点（必须保留）

1. Network.loadingFailed 处理（`recv_navigation_status` 立即返回错误）— Task 1 已实现
...
**重要**：原计划文档中使用 `self.cf_jar.get(domain)` 和 `self.cf_jar.insert(domain, ...)`，
但实际 `CfCookieJar` 的 API 方法名是 `get_session` 和 `insert_session`。下方代码已修正。

参考 `src/cookie/cf.rs`：
- `pub fn get_session(&self, domain: &str) -> Option<CfSession>`
- `pub fn insert_session(&self, domain: String, session: CfSession)`

`CfSession` 结构（`src/cookie/cf.rs`）：
```rust
pub struct CfSession {
    pub cookies: Vec<serde_json::Value>,  // CDP 原始 cookie JSON 数组
    pub ua: String,
    pub saved_at: i64,
}
```
...
创建 `src/fetcher/strategies/stealth.rs`，包含完整实现 + 测试（代码已修正 API 调用）：
...
1. 修改 `src/fetcher/strategies/mod.rs`，启用 stealth 行：
...
2. 修改 `src/fetcher/mod.rs`，在 `pub mod strategies;` 后添加 re-export：
...
```bash
git add src/fetcher/strategies/stealth.rs src/fetcher/strategies/mod.rs src/fetcher/mod.rs
