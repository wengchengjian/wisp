...
fn meta_edge_cases_empty_collections_and_bools() {
    ...
    assert!(
        restored.meta["empty_obj"].is_object(),
        "空对象应保持对象类型"
    );
    assert!(
        restored.meta["empty_obj"].as_object().unwrap().is_empty(),
        "空对象应为空"
    );
    assert!(
        restored.meta["empty_arr"].is_array(),
        "空数组应保持数组类型"
    );
    assert!(
        restored.meta["empty_arr"].as_array().unwrap().is_empty(),
        "空数组应为空"
    );
